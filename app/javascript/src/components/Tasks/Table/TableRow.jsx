      ...
      {data.map(rowData => (
        <tr key={rowData.id}>
          <td className="text-center">
            <input
              checked={isCompleted}
              className="rounded focus:ring-bb-purple ml-6 h-4 w-4 cursor-pointer border-gray-300 text-bb-purple"
              type="checkbox"
              onChange={() =>
                handleProgressToggle({
                  slug: rowData.slug,
          ...
          {isCompleted && (
            <>
              ...
              <td className="cursor-pointer py-4 pl-6 text-center">
                <i
                  className="transition ri-delete-bin-5-line text-center text-2xl text-bb-border
                  duration-300 ease-in-out hover:text-bb-red"
